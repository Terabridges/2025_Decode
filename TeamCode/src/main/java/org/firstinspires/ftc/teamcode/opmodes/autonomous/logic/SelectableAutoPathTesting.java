...
public class SelectableAutoPathTesting extends SelectableOpMode {
    ...
    public SelectableAutoPathTesting() {
        super("Select Auto Path Test", c -> {
            c.folder("Blue Alliance", p -> {
                p.folder("Close", m -> {
                    m.add("Main Release (R: P, 2, close loop, 1)", () -> make(Alliance.BLUE, closeMainReleaseSpec()));
                ...
                p.folder("Far", m -> {
                    ...
                    m.folder("Other", s -> {
                        s.add("Back Row Only (R: P, 4, backrow loop)", () -> make(Alliance.BLUE, FAR_BACKROW_ONLY));
                        s.add("Rows 3 + 2 (R: P, 3, 2)", () -> make(Alliance.BLUE, FAR_3_THEN_2));
            ...
            c.folder("Red Alliance", p -> {
                p.folder("Close", m -> {
                    m.add("Main Release (R: P, 2, close loop, 1)", () -> make(Alliance.RED, closeMainReleaseSpec()));
                ...
                p.folder("Far", m -> {
                    ...
                    m.folder("Other", s -> {
                        s.add("Back Row Only (R: P, 4, backrow loop)", () -> make(Alliance.RED, FAR_BACKROW_ONLY));
                        s.add("Rows 3 + 2 (R: P, 3, 2)", () -> make(Alliance.RED, FAR_3_THEN_2));
    ...
    private static AutoSpec closeMainReleaseSpec() {
        return AutoSpec.withCloseLoopCycles(
                Range.CLOSE_RANGE,
                true,
                true,
                CloseMainReleaseAuto.closeLoopCycles,
                2,
                1
        );
    }
