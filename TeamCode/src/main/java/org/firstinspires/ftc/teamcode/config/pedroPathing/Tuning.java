    ...
    public Tuning() {
        super("Select a Tuning OpMode", s -> {
            ...
            s.folder("Tests", p -> {
                ...
                p.add("Ninety Degree w/ Predictive", Line90Degree::new);
                p.add("Ninety Degree w/o Predictive", Line90DegreeNoPredictive::new);
            });
        });
    }
...
class Line90Degree extends OpMode {
    ...
    public void start() {
        follower.activateAllPIDFs();
        forwards = new Path(new BezierLine(new Pose(0,0), new Pose(DISTANCE*2,0)));
        forwards.setConstantHeadingInterpolation(0);
        sideways = new Path(new BezierLine(new Pose(DISTANCE*2,0), new Pose(DISTANCE*2,
                DISTANCE)));
        sideways.setConstantHeadingInterpolation(0);
        chain = new PathChain(forwards, sideways);
...
class Line90DegreeNoPredictive extends OpMode {
    public static double DISTANCE = 24;
    private boolean forward = true;
    ...
    public void start() {
        ...
        follower.constants.usePredictiveBraking = false;
        follower.followPath(chain);
    }
