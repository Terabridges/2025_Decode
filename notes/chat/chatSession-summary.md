...
### “Free with bulk read” vs “extra cost” (verified map)

Validated against FTC SDK 11.0.0 Lynx source under `LogTest/build/tmp/ftc-hw-11-sources` and current PsiKit wrappers.

PsiKit behavior today:
- Hubs are in `MANUAL` bulk mode and `clearBulkCache()` is called once per loop.
- Optional `FtcLogTuning.prefetchBulkDataEachLoop` can intentionally pull one bulk packet per hub early in the loop.

Bulk-backed on REV Lynx hubs (when bulk caching is not `OFF`):
- `DcMotor.getCurrentPosition()`
- `DcMotor.isBusy()` (internally checks `LynxIsMotorAtTargetCommand`, bulk-backed when enabled)
...
Not bulk-backed (separate transaction or cached software value):
- `DcMotor.getPower()` (constant-power command/readback path)
- `DcMotor.getTargetPosition()`
- `DcMotorEx.getCurrent(CurrentUnit)` (ADC command)
- `DcMotorEx.getCurrentAlert(...)` (last-known cached value unless refreshed)
- Typical servo readbacks (`getPosition()`, PWM enabled/range) are not Lynx bulk-backed

Important nuance:
- “Bulk-backed” means data can come from the hub’s cached bulk packet after first access that loop.
- It is not free overall: Java getter calls, wrapper logic, and log table writes still add per-device CPU time.
